...
An Anonymous Token with Hidden Metadata (ATHM), as specified in {{!ATHM-SPEC=I-D.yun-cfrg-athm}}, allows the issuer to embed
...
The ATHM tokens provide a way to enable hidden trust signals in an authentication token, while achieving the
...
an anonymous credential construction called ATHM, as specified in {{ATHM-SPEC}}.
